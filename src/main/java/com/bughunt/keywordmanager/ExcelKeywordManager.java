			...
			while(cellIt.hasNext()) {
				...
				if(StringUtils.isNotEmpty(methodName)) {
					if(TestSession.getKeywordMap().containsKey(methodName)) {
						...
						String message = String.format("Keyword '%s' is not implemented which is required for test '%s'.\nPlease implement this keyword.", methodName, tcName);
						throw new InCompleteSettingsException(message);
					}
				} else {
					break;
				}
